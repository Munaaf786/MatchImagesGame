...
class App extends Component {
  ...
  renderTabItems = (tabItem, tabId) => {
    const {category} = this.state
    return (
      <li className="tabItem" key={tabId}>
        <button
          type="button"
          onClick={() => this.onClickTabItem(tabId)}
          className={`tabItem-button ${
            category === tabItem.tabId ? 'active-tab' : ''
          }`}
        >
          {tabItem.displayText}
        </button>
  ...
  onClickImage = imageDetails => {
    const {imgUrl} = this.state
    if (imgUrl === imageDetails.imageUrl) {
      const newImageUrl =
      ...
      clearInterval(this.timerId)
      this.setState({showResults: true, time: 0})
    }
  }

  renderImages = (imageDetails, id) => (
    <li key={id}>
      <button
        type="button"
        className="imageItem"
        onClick={() => this.onClickImage(imageDetails)}
      >
        <img
  ...
  render() {
    const {time, category, imgUrl, score, showResults} = this.state
          ...
          <ul className="score-and-time">
            ...
            <li className="score-time">
              <img
                ...
                className="timer"
              />
              <p className="highlight">{time} sec</p>
            </li>
          </ul>
        </div>
        <div className="body">
          {!showResults && (
            <>
              <img src={imgUrl} alt="match" className="img-to-be-matched" />
              <ul className="tabsList">
                {tabsList.map(tabItem =>
                  this.renderTabItems(tabItem, tabItem.tabId),
                )}
              </ul>
              <ul className="imagesList">
                {filteredImageList.map(imageDetails =>
                  this.renderImages(imageDetails, imageDetails.id),
                )}
              </ul>
            </>
          )}
          {showResults && (
            <div className="results-page">
              <img
                ...
                className="trophy"
              />
              <p className="your-score-label">YOUR SCORE</p>
